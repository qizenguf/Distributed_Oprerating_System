...
class pastryNode(pubKey:Int,Pa:Array[pastryNode],Boss:Actor,nodeID:String,numrequest:Int) extends Actor {
    ...
    loop{
	 ...
	 case "close" => exit()
        ...
        case route(rtID, hop)=>
	     ...
	     var f = 1
          ...
          if(rtID==nodeID) {  Boss ! HOP(hop); 
                              println("0 Msnum :"+Msnum.toString+"  with hop= "+hop.toString+"  in  "+pubkey.toString)
          	               // println("here over")
                            //   self ! "route"; 
                            }
            else{
           // whether in lfSet 	
            while(i < lfSet.length && f==1) {
                ...
                {
                  Boss ! HOP(hop+1)
                  println("1 Msnum :"+Msnum.toString+"  with hop= "+hop.toString+"  in  "+pubkey.toString)
          ...
          if(nextHop!=pubKey) {
          	...
          	Boss ! HOP(hop)
                println("2 Msnum :"+Msnum.toString+"  with hop= "+hop.toString+"  in  "+pubkey.toString)
          //	self ! "route"; 
            }
          }
        }
        ...
        // route message to self with node ID
          ...
          if (Msnum==numrequest+1){
             Boss ! "finish";Msnum+=1;
             println(pubkey.toString+"is closed")
                } 
  ...
  def broadCast{
   ...
   def updateWithNew(rtID: String, key:Int){
     ...
     if(!comp(rtID,nodeID)){
     	if (lfSet((BASE/2)-1)!=null) {if(Dis(lfSet(findFar(0))._1,nodeID)>Dis(rtID,nodeID)) lfSet(findFar(0))=newNode}
     	else{
     		f=1;i=0;
       	while(i < BASE/2 && f==1) {
		        if(lfSet(i)==null)  
      ...
      react{
        ...
        if (readyNode==numNodes-1)    {     
         ...
         for( i <- 0 until numNodes) {
        	      Pa(i) ! "route" 
        	    //  println(i.toString)
               // Pa(i).lfSet   foreach{a => if(a!=null) println(a._1+" + "+a._2.toString)};
                //Pa(i).neSet   foreach{a => if(a!=null) println("n:"+a._1+"+"+a._2.toString)};
            }
         val t=new timer(500,Pa,numNodes, numrequest)
             t.start
         }
         else {      
          readyNode+=1;    
       ...
       // println(ha)
        ...
        case "finish" =>
        endNode+=1;
       println("end = :"+endNode.toString)
        if (endNode>=numNodes){
        	println(sum.toString+"sum : n "+n.toString)
        println("result:"+(sum.toDouble/n.toDouble).toString);		
       // println("compared with :"+)
        ...
        case HOP(hop)=>
		      sum+=hop;
		       n+=1;
		      // println("n= "+n.toString+" sum= "+sum.toString)
...
class timer(time:Int,Pa:Array[pastryNode],numNodes:Int, numrequest:Int) extends Actor {
def act{
